# src/performa_video/utils/minio_rollback.py
"""MinIO point-in-time rollback for the performa_video ETL.
...
or via this module:
    python -m src.performa_video.utils.minio_rollback --enable-versioning
...
- The 'full' scope additionally purges the failed run's parquet/quarantine/
  manifest artifacts for BOTH datasets, but only versions in the
  [before_ts .. now] window, so a same-day sibling run's earlier versions are
  never touched.
"""
...
# Watermark object + prefixes this ETL owns. The watermark scope covers the
# whole "watermarks/" prefix via prefix-scan, restoring BOTH
# watermarks/performa_video.json AND watermarks/produksi.json in one shot.
...
def _scope_prefixes(scope: str, run_key: str | None = None) -> list[str]:
    ...
    This project runs BOTH datasets (video + produksi) in one run, so the
    'full' scope covers both parquet prefixes, both quarantine subfolders and
    both fix-manifest subfolders, plus the shared error manifest prefix.
    """
    ...
    if scope == "full":
        ...
        day = run_key[:8]
        prefixes += [
            f"performa/video/date={day}/",
            f"produksi/date={day}/",
            f"quarantine/date={day}/",
            f"quarantine/video/date={day}/",
            f"quarantine/produksi/date={day}/",
            "error_list_watermark/",
            f"fix_error_list_watermark/video/date={day}/",
            f"fix_error_list_watermark/produksi/date={day}/",
        ]
    return prefixes
...
def auto_restore_watermark(client: Minio, bucket: str, run_key: str) -> str:
    ...
    Scope: the watermark prefix ONLY (both performa_video.json and
    produksi.json — never touches parquet / quarantine).
...
def _cli_client(args) -> tuple[Minio, str]:
    if args.endpoint or args.access or args.secret or args.bucket:
        ...
        from src.performa_video.utils.minio_client import get_minio_client
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="MinIO point-in-time rollback for performa_video "
    ...
    if not bucket_versioning_enabled(client, bucket):
        ...
        print("  python -m src.performa_video.utils.minio_rollback --enable-versioning")
        return 2
